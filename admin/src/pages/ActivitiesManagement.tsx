...
const ActivitiesManagement: React.FC = () => {
  ...
  const openModal = (activity?: Rifa) => {
    if (activity) {
      ...
      setFormData({
        ...
        precio_boleto: toNumber(activity.precio_boleto),
        ...
        cantidad_numeros_suerte: activity.cantidad_numeros_suerte || 5,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingActivity) {
        ...
        const updatedActivity = await apiService.updateActivity(editingActivity.id.toString(), formData);
        setActivities(prev => prev.map(activity =>
          activity.id === editingActivity.id ? updatedActivity.activity : activity
        ));
      } else {
        // Create new activity
        const newActivity = await apiService.createActivity(formData);
  ...
  const deleteActivity = async (id: number) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar esta actividad?')) {
      try {
        ...
        const errorMessage = error.data?.message || error.message || 'Error al eliminar la actividad';
  ...
  const drawActivity = async (id: number) => {
    if (window.confirm('¿Estás seguro de que deseas realizar el sorteo? Esta acción no se puede deshacer.')) {
      try {
        setSubmitting(true);
        await apiService.drawActivity(id.toString());
  ...
  const assignMainWinner = async (id: number) => {
    if (window.confirm('¿Asignar ganador principal a esta actividad?')) {
      try {
        setSubmitting(true);
        const result = await apiService.assignMainWinner(id);
        console.log('Main winner assigned:', result);
        await loadActivities();
      } catch (error) {
        console.error('Error assigning main winner:', error);
        setError('Error al asignar ganador principal');
      } finally {
        setSubmitting(false);
      }
    }
  ...
  const markAsFinished = async (id: number) => {
    if (window.confirm('¿Marcar esta actividad como finalizada?')) {
      try {
        setSubmitting(true);
        const result = await apiService.markAsFinished(id);
        console.log('Activity marked as finished:', result);
        await loadActivities();
      } catch (error) {
        console.error('Error marking as finished:', error);
        setError('Error al marcar como finalizada');
      } finally {
        setSubmitting(false);
      }
    }
    ...
    return (
      <div className="activities-management">
        <div className="page-header">
          ...
          <p className="page-subtitle">Cargando actividades...</p>
        </div>
            ...
            <tbody>
              {activities.map((activity) => (
                <tr key={activity.id}>
                  ...
                  <td>{formatCurrency(toNumber(activity.precio_boleto))}</td>
                  <td>
                    <div className="progress-cell">
                      <div className="progress-bar-small">
                        <div 
                          className="progress-fill-small"
                          style={{ width: `${toNumber(activity.porcentaje_vendido)}%` }}
                        />
                      </div>
                      <div className="progress-text">
                        {activity.boletos_vendidos}/{activity.total_boletos} 
                        ({(toNumber(activity.porcentaje_vendido) || 0).toFixed(1)}%)
                      </div>
                    </div>
                  </td>
                  <td>
                    <div className="lucky-numbers">
                      <div className="numbers-count">
                        {activity.cantidad_numeros_suerte || 0} números
                      </div>
                      {activity.numeros_premiados && activity.numeros_premiados.length > 0 && (
                        <div className="numbers-preview">
                          {activity.numeros_premiados.slice(0, 2).join(', ')}
                          {activity.numeros_premiados.length > 2 && '...'}
                        </div>
                      )}
                    </div>
                  </td>
                  ...
                  <td>
                    <div className="table-actions">
                      <button 
                        ...
                        onClick={() => navigate(`/activities/${activity.id}/lucky-numbers`)}
                        title="Ver Números de Suerte"
                      >
                        🎯
                      </button>
                      <button 
                      ...
                      {activity.estado === 'activa' && toNumber(activity.porcentaje_vendido) >= 100 && (
                        <button 
                      ...
                      {((activity as any).winners && (activity as any).winners.length > 0 && !(activity as any).main_winner) && (
                        <button 
                          className="action-btn orange"
                          onClick={() => assignMainWinner(activity.id)}
                          title="Asignar Ganador Principal"
                        >
                          👑
                        </button>
                      )}
                      {((activity as any).main_winner && activity.estado === 'sorteo_en_curso') && (
                        <button 
                          className="action-btn teal"
                          onClick={() => markAsFinished(activity.id)}
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <form onSubmit={handleSubmit} className="activity-form">
              ...
              <div className="form-group">
                <label>Cantidad de Números de Suerte *</label>
                <input
                  type="number"
                  name="cantidad_numeros_suerte"
                  value={formData.cantidad_numeros_suerte}
                  onChange={handleInputChange}
                  min="1"
                  max="20"
                  required
                ...
                <small className="form-help">
                  Cantidad de números aleatorios que se generarán automáticamente para esta actividad
                </small>
              </div>
              ...
              <div className="modal-actions">
                ...
                <button type="submit" className="btn btn-primary" disabled={submitting}>
                  {submitting ? 'Guardando...' : editingActivity ? 'Actualizar' : 'Crear'}
                </button>
              </div>
            </form>
          </div>
